...
import { createCourse, createRun, getCourse, getRelease, getRun, listArtifacts, listCourses, listReleases, postAction, postReview } from "./api";
...
function Workbench() {
  ...
  const artifacts = useQuery({ queryKey: ["artifacts", courseId], queryFn: () => listArtifacts(courseId), refetchInterval: active(run.data?.status) ? 2_000 : false });
